...
def main(_):
	...
	# for run_idx in range(num_run):
		...
		mri_train, mri_test, label_train, label_test = rdfa.read_npy_file_3d() # norm
		print('TRAIN, TEST: ', np.shape(mri_train), np.shape(mri_test))
		...
		if 1:
			...
			if 1:
				...
				params = {'encoding_dim':encode_dim}
				ini_weight, ini_weight2 = test_simple_ae(mri_train, mri_test, params)
		...
		W_fc3 = weight_variable([num_hid_units, num_hid2])
		b_fc3 = bias_variable([num_hid2])
		h_fc3 = tf.nn.relu(tf.matmul(h_fc1, W_fc3) + b_fc3)

		W_fc4 = weight_variable([num_hid2, num_hid3])
		b_fc4 = bias_variable([num_hid3])
		h_fc4 = tf.nn.relu(tf.matmul(h_fc3, W_fc4) + b_fc4)
		
		keep_prob = tf.placeholder(tf.float32)
		h_fc1_drop = slim.dropout(h_fc4, keep_prob)
		
		W_fc2 = weight_variable([num_hid3, 2])
	  ...
	  # Define loss and optimizer
		...
		# all_acc_all_run = all_acc_all_run+ [np.mean(resl)]
